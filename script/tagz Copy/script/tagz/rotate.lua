...
function init()
    SetBool("onsvrg.interactions.rotate",true)
    SetBool("onsvrg.rotate.refresh",true)
	refresh()
end

function tick(dt)
    if GetBool("onsvrg.rotate.refresh") then
        refresh()
    end
	...
	for id,value in pairs(data) do
        ...
        if data[id].active then  
            SetJointMotor(data[id].handle , data[id].speed * 2 * math.pi)
        else
            SetJointMotor(data[id].handle , 0)
        end
	end
end

function refresh()
    ids = FindJoints("rotate",true)
    for i = 1, #ids do
        local id = ids[i]
        local delay = 0
        ...
        data[id].active = true
        data[id].speed = 1
        if GetTagValue(data[id].handle,"rotate") ~= "" then
    ...
    SetBool("onsvrg.rotate.refresh",false)
end
